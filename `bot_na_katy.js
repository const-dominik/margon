...
((parseInput, _g, npctalk) => {
    ...
    //const MAPKI = ["Pustynne Katakumby", "Pustynne Katakumby - sala 1", "Komnaty Bezdusznych - sala 1", "Komnaty Bezdusznych - sala 2", "Komnaty Bezdusznych - sala 1", "Pustynne Katakumby - sala 2"];
    const MAPKI = ["Katakumby Gwałtownej Śmierci", "Korytarz Porzuconych Marzeń", "Katakumby Opętanych Dusz", "Katakumby Odnalezionych Skrytobójców", "Korytarz Porzuconych Nadziei", "Katakumby Opętanych Dusz", "Zachodni Tunel Jaźni", "Katakumby Poległych Legionistów", "Zachodni Tunel Jaźni", "Katakumby Krwawych Wypraw", "Wschodni Tunel Jaźni"];
    ...
    const findBestMob = () => {
        ...
        if (window.g.npc) {
            for (const npc in window.g.npc) {
                if (!ignoreId.includes(npc) && [2, 3].includes(window.g.npc[npc].type) && checke2(npc) && window.g.npc[npc].lvl > 250 && (window.g.npc[npc].wt <= 19 || window.g.npc[npc].wt > 79) && heuristic(window.hero, window.g.npc[npc]) < droga2) {
                    if (window.g.npc[npc].wt>79 && g.other.length !== 0 && (new Date).getDay() !== 1) {
                        ignoreId.push(npc);
                        return findBestMob();
                    }
    ...
    const sendToDiscord = (kto, uKogo, pieniadz) => {
        fetch("https://discordapp.com/api/webhooks/674979684064165898/2m3Ef7HyoFbFgJjpKcTe9mAFRSqWqjYvLhC0em-2HvTetuOyDSX8bxC8dsUsEPqmGs84", {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    embeds: [{
                        title: "Nowe pieniążki",
                        description: `${uKogo === "Dom Tunii" ? "Tunia" : "Kendal"} - zarobiłem ${Math.round(pieniadz/1000)/1000}m.
                        Mam: ${Math.round(window.hero.gold/1000000)/1000}g.`
                    }],
                    username: kto
                }),
            });
    ...
    const toSell = id => {
        ...
        let nieSell = ["legendarny", "ulepszony", "Klucze", "Konsumpcyjne", "Torby", "Talizmany", "Związany z właścicielem", "Złoto", "Mikstury", "Questowe"];
    ...
    const sellanie = () => {
        if (document.querySelector("#shop")) {
            let itemy = [];
            let tpId, tpAmount, potId, potCount = 0;
            for (let [id, {
                    name,
                    loc,
                    stat
                }] of Object.entries(window.g.item)) {
                if (toSell(id) || (window.map.name === "Dom Tunii" && name.includes("imperialnego"))) itemy.push(id);
                if (loc === "g") {
                    if (name === "Zwój teleportacji na Kwieciste Przejście") {
                        tpAmount = parseItemStat(stat).amount;
                    } else if (name === "Duża pomarańczowa mikstura") {
                        potCount++;
                    }
                }
            }
            const gold = window.hero.gold;
            if (itemy.length) {
            setTimeout(() => {
                window._g(`shop&buy=&sell=${itemy.length ? itemy.join(",") : ""}`, res => {
                    for (let [id, {
                        ...
                        else if (name === "Duża pomarańczowa mikstura" && loc === "n") potId = id;
                    const toBuy = [];
                    if (tpAmount <= 10) toBuy.push(`${tpId},1`);
                    if (potCount === 1) toBuy.push(`${potId},1`);
                    else if (potCount === 0) toBuy.push(`${potId},2`);
                    setTimeout(() => {
                        window._g(`shop&buy=${toBuy.join(";")}&sell=`, res => {
                            sendToDiscord(window.hero.nick, window.map.name, window.hero.gold-gold);
                            window.shop_close(true);
                            if (hero.gold/1000000>=50) _g("clan&a=save&f=gold&v=50000000");
                            window.map.name === "Dom Tunii" ? localStorage.setItem("job", "kendal") : localStorage.setItem("job", "wracanie");
                            controller();
                        });
                    }, 500);
                });
            ...
            window.map.name === "Dom Tunii" ? canTalkOrWalkAndTalk(getNpcId("Tunia Frupotius")) : canTalkOrWalkAndTalk(getNpcId("Kowal Kendal"));
        }
    }
    ...
    const controller = () => {
        if (g.dead) {
            localStorage.setItem("job", "kendal");
            setTimeout(controller, 30000);
        }
        const THUZAL = ["Dom Tunii", "Kwieciste Przejście", "Lazurowe Wzgórze", "Grań Gawronich Piór", "Thuzal", "Gildia Magów"];
        const KENDAL = ["Tuzmer", "Port Tuzmer", "Kuźnia Kendala"];
        //const KATY = ["Kuźnia Kendala", "Port Tuzmer", "Wioska Rybacka", "Ciche Rumowiska", "Oaza Siedmiu Wichrów", "Ruiny Pustynnych Burz", "Pustynne Katakumby"]
        const KATY = ["Kuźnia Kendala", "Port Tuzmer", "Wioska Rybacka", "Ciche Rumowiska", "Oaza Siedmiu Wichrów", "Ruiny Pustynnych Burz", "Pustynne Katakumby", "Pustynne Katakumby - sala 2", "Komnaty Bezdusznych - sala 1", "Komnaty Bezdusznych - sala 2", "Katakumby Gwałtownej Śmierci"]
        if (THUZAL.concat(KENDAL).concat(KATY).includes(window.map.name) || "Trupia Przełęcz" === window.map.name) {
            ...
            if (job === "tunia") {
                ...
                if (THUZAL.includes(window.map.name)) {
                    const mapka = THUZAL.indexOf(window.map.name);
                    if (THUZAL[mapka + 1]) enterLocation(THUZAL[mapka + 1]);
                    else if (THUZAL[mapka] === THUZAL[THUZAL.length - 1]) canTalkOrWalkAndTalk("Fontanna Planów Astralnych");
                } else if (KENDAL.includes(window.map.name)) {
                    const mapka = KENDAL.indexOf(window.map.name);
                    if (KENDAL[mapka + 1]) enterLocation(KENDAL[mapka + 1]);
                    else if (KENDAL[mapka] === KENDAL[KENDAL.length - 1]) canTalkOrWalkAndTalk("Kowal Kendal");
                } else if (window.map.name === "Trupia Przełęcz") {
                    canTalkOrWalkAndTalk("Astralny koncentrator");
                }
    ...
    window.npcTalk = d => {
        ...
        switch (d[1]) {
            case "Tunia Frupotius":
                ...
                break;
            case "Kowal Kendal":
                if (d.includes("Słyszałem, że na zapleczu masz coś, co mogłoby mi się przydać. ")) return cTalk(d, 8, setTimeout(sellanie, 500));
                break;
            case "Fontanna Planów Astralnych":
                if (d.includes("Trupia Przełęcz [Tuzmer] (5000 sztuk złota). ")) return cTalk(d, 26);
                break;
            case "Astralny koncentrator":
                if (d.includes("Tuzmer (5000 sztuk złota). ")) return cTalk(d, 26);
                break;
        }
    }
